 ...
 * @param element 模块元素
 */
export const onMove = (name: string, element: HTMLElement) => {
  // myMove.init(element, configName, name);
  element.onmousedown = async (ev) => {
    // const pfConfig = await myStorage.getConfig();
    if ((element.querySelector('.lock-icon') as HTMLElement).dataset.lock === 'true') {
      return;
    }
