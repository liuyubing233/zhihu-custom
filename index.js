...
// @version      1.7.10
// @description  一键极简模式，去除不必要的元素（可自动配置，随时还原），给你最简单的知乎；设置过滤烦人的故事档案局和盐选科普回答，并可一键过滤所有知乎官方账号回答；首页切换模块，发现切换模块、个人中心、搜素栏可悬浮并自定义位置；支持版心修改，页面模块位置调整、隐藏，页面表头和图标修改；页面背景色修改，黑色为夜间模式；列表的问题，文章和视频添加区分标签；去除广告，外链直接打开；更多功能请在插件里体验；想要的最终功能是页面大部分模块全可配置，目前努力更新中...
// @author       pufferfish
// @match         *://www.zhihu.com/*
...
(function () {
  ...
  function initData() {
    ...
    $('.pf-modal-content')[0].onchange = (even) => {
      if ($(even.target).hasClass('pf-input')) {
        return throttle(myChanger(even.target, even.target.type))
      }
    }
  ...
  function initColorsList() {
    ...
    if (myLocalC.firstInitColors) {
      ...
      Object.keys(pfConfig).forEach((key) => {
        /^colors/.test(key) && initColorHTML(key, pfConfig[key])
      })
    } else {
      Object.keys(pfConfig).forEach((key) => {
        if (/^colors/.test(key)) {
          // 如果配置中的颜色在缓存中不相等，则重新加载颜色元素
          pfConfig[key] !== myLocalC.cacheColors[key] && initColorHTML(key, pfConfig[key])
        }
      })
    }
  }

  function initColorHTML(key, colors) {
  ...
  function initCSSVersion() {
    const cssVersion = '<style type="text/css" id="pf-css-version">'
      + `.QuestionHeader .QuestionHeader-content,.QuestionHeader-footer .QuestionHeader-footer-inner,.QuestionHeader-content,.Question-main,.AppHeader-inner,.TopstoryPageHeader,.Topstory-container,.ExploreHomePage,.QuestionWaiting,.SearchTabs-inner,.Search-container,.ProfileHeader,.Profile-main,.CollectionsDetailPage,.ColumnPageHeader-content,.SettingsMain,.App-main .Creator,.Collections-container,.Balance-Layout,.zhuanlan .Post-NormalMain .Post-Header,.zhuanlan .Post-RichTextContainer,.zhuanlan .Post-NormalMain>div,.zhuanlan .Post-content .RichContent-actions,.zhuanlan .Post-NormalSub>div{width:${vHeart().v}!important;}`
      ...
      + `img.lazy,.GifPlayer img,.ArticleItem-image,.ztext figure .content_image, .ztext figure .origin_image,.TitleImage{${vImgSize()}}.GifPlayer.isPlaying img{cursor:pointer!important;}`
      + `${hiddenCSS()}`
  ...
  function shoppingLinkCSS() {
    const shopCSSObj = {
      'default': '',
      'justText': '.MCNLinkCard-imageContainer,.MCNLinkCard-button,.MCNLinkCard-source,.ecommerce-ad-commodity-img,.ecommerce-ad-commodity-box-icon{display: none!important;}'
        + '.MCNLinkCard,.MCNLinkCard-card,.ecommerce-ad-commodity{min-height: 0!important;background: transparent!important;width:100%!important;max-width:100%!important;}'
        ...
        + '.MCNLinkCard-titleContainer,.ecommerce-ad-commodity-main-content-des span{color: #06f!important;justify-content: start!important;}'
        + '.MCNLinkCard-titleContainer::before,.ecommerce-ad-commodity-main-content-des span::before{content: "购物链接："}.MCNLinkCard-title{color: #06f!important;}',
  ...
  function hiddenCSS() {
    return `${pfConfig.hiddenAnswerRightFooter ? '.Question-sideColumn{display: none!important;}' : ''}`
      ...
      + `${pfConfig.hiddenAnswers ? '.Topstory-container .RichContent.is-collapsed .RichContent-inner,.HotItem-excerpt--multiLine,.TopstoryQuestionAskItem .RichContent .RichContent-inner,.HotItem-content .HotItem-excerpt{display: none;}' : ''}`
      ...
      + `${pfConfig.hiddenDetailAvatar ? '.AnswerItem .AuthorInfo .AuthorInfo-avatarWrapper{display: none;}.AnswerItem .AuthorInfo .AuthorInfo-content{margin-left:0!important;}' : ''}`
  ...
  const myBackground = {
    ...
    transparentBackground: () => {
      return `.zhuanlan .RichContent-actions.is-fixed,.AnnotationTag,.ProfileHeader-wrapper{background-color: transparent!important;}`
    }
  }
  ...
  function initHTML() {
    ...
    // 添加EVENT
    $('.pf-open-modal')[0] && ($('.pf-open-modal')[0].onclick = myDialog.open)
  ...
  function storyHidden() {
    if (pfConfig.removeStoryAnswer || pfConfig.removeYanxuanAnswer || pfConfig.removeFromYanxuan || pfConfig.removeZhihuOfficial) {
      ...
      for (let i = eachIndex; i < events.length; i++) {
        const that = events[i]
