...
// @version      2.0.3
// @description  ✈持续更新✈努力实现功能最全的知乎配置插件|列表种类和关键词强过滤内容|可设置收起所有长回答或展开所有回答|未登录状态下问答和专栏移除登录弹窗|过滤烦人的故事档案局和盐选科普回答，可一键过滤所有知乎官方账号回答|首页切换模块，发现切换模块、个人中心、搜素栏可悬浮并自定义位置|支持版心修改，页面模块位置调整、隐藏，页面标头和图标修改|页面背景色修改|列表的问题，文章和视频添加区分标签|去除广告，可设置购买链接只显示文字还是隐藏，外链直接打开|更多功能请在插件里体验
...
(function () {
  ...
  function initData() {
    ...
    if (isLoading) {
      isLoading = false
      console.log('[customize]修改器加载完毕，加载时间：' + (performance.now() - timeStart).toFixed(2) + 'ms')
    }
  }
        ...
        if (that) {
          if (pfConfig.removeZhihuOfficial) {
            ...
            if (/知乎[\s]*官方帐号/.test(label)) {
              $(that).remove()
              lessNum++
              console.log(`[customize]已删除一条知乎官方帐号的回答`)
            }
          } else {
            ...
            removeAnswerByAuthorName.forEach((item) => {
              ...
              if (pfConfig[item.id] && reg.test(dataZop)) {
                $(that).remove()
                lessNum++
                console.log(`[customize]已删除一条${item.name}的回答`)
              }
            })
          }
          ...
          if (pfConfig.removeFromYanxuan) {
            ...
            if (formYanxuan) {
              ...
              if (/盐选专栏/.test(formYanxuanText)) {
                $(that).remove()
                lessNum++
                console.log(`[customize]已删除一条来自盐选专栏的回答`)
              }
            }
          }
  ...
  function filterItemByKeyword() {
    ...
    for (let i = filterIndex, len = events.length; i < len; i++) {
      ...
      if (isFindTitle) {
        ...
        lessNum++
        // 标识console 勿删
        console.log(`[customize]关键词过滤，内容标题：${title}`)
        console.log(`[customize]已调用过滤接口`)
        $(events[i]).parent().parent().remove()
      } else if (typeKey && pfConfig[typeKey]) {
        $(events[i]).parent().parent().remove()
        lessNum++
        console.log('[customize]---列表种类过滤---')
      }
