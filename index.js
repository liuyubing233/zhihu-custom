...
如果开启了「不再显示已屏蔽用户发布的内容」那么也不会看到对方发布的回答`;
  ...
  var initInputRange = () => {
    ...
    const versionCallback = (item, index) => {
      return createRangeInnerHTML(item.label, item.value, item.min, item.max) + createRangeInnerHTML(item.percentLabel, item.percentValue, item.percentMin, item.percentMax) + `<label class="ctz-flex-wrap">
        <span class="ctz-label">${item.percentChooseLabel}</span>
        <input class="ctz-i" name="${item.percentChooseValue}" type="checkbox" value="on" />
      </label><div class="ctz-commit" style="${index < VERSION_RANGE.length - 1 ? "border-bottom: 1px solid #e0e0e0;" : "margin:0;"}padding:8px 0;"><b>${item.desc}</b></div>`;
    };
  ...
  var myListenListItem = {
    ...
    init: async function() {
      ...
      for (let i = this.index, len = elements.length; i < len; i++) {
        ...
        (isVideo && removeItemAboutVideo || isArticle && removeItemAboutArticle || isTip && removeItemAboutPin) && !message && (message = `列表种类屏蔽，${nodeItemContent.classList.value}`);
