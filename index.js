...
如果开启了「不再显示已屏蔽用户发布的内容」那么也不会看到对方发布的回答`;
  ...
  var commonFormBoxItem = (con) => con.map(
    (item) => `<div class="ctz-form-box">${item.map(
      ({ label, value, needFetch }) => `<div class="ctz-form-box-item ${needFetch ? "ctz-fetch-intercept" : ""}">${`<div>${label + (needFetch ? '<span class="ctz-need-fetch">（接口拦截已关闭，此功能无法使用）</span>' : "")}</div><div><input class="ctz-i ctz-switch" name="${value}" type="checkbox" value="on" /></div>`}</div>`
    ).join("")}</div>`
  ).join("");
  var initHTML = () => {
