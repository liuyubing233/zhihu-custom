...
屏蔽后，对方将不能关注你、向你发私信、评论你的实名回答、使用「@」提及你、邀请你回答问题，但仍然可以查看你的公开信息。`;
  ...
  var initBlackList = async () => {
    ...
    nodeBlockedUsersTags.innerHTML = blockedUsersTags.map(
    ...
    nodeBlockedUsersTags.onclick = (event) => {
      ...
      const isUsed = blockedUsers.some((item2) => {
        if (item2.types && item2.types.length) {
          return item2.types.some((i) => i === info);
        }
        return false;
      });
      if (isUsed) {
    ...
    dom('input[name="inputBlockedUsersTag"]').onchange = async (e) => {
      const { blockedUsersTags: blockedUsersTags2 = [] } = await myStorage.getConfig();
      const target = e.target;
      const value = target.value.toLowerCase();
      if (blockedUsersTags2.includes(value)) {
        message("该标签已经存在");
        return;
      }
      blockedUsersTags2.push(value);
      await myStorage.updateConfigItem("blockedUsersTags", blockedUsersTags2);
