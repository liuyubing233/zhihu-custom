...
如果开启了「不再显示已屏蔽用户发布的内容」那么也不会看到对方发布的回答`;
  ...
  var createHiddenItem = (arrHidden) => {
    ...
    return `<div class="ctz-checkbox-group">${arrHidden.map((item) => item.map((i) => `<label style="display: inline-flex;" class="ctz-checkbox">${commonCheckbox(i.value)}<div>${i.label}</div></label>`).join("")).join("<br>")}</div>`;
  };
