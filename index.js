...
(function () {
  ...
  function addBlockUser(event) {
    ...
    try {
      ...
      buttonBlockUser[0].onclick = function () {
        ...
        isUse && doFetchActionsBlock(userUrl, userName)
      }
  ...
  /** 调用「屏蔽用户」接口 */
  function doFetchActionsBlock(userUrl, userName) {
    const urlName = userUrl.match(/(?<=people\/)[\w\W]+/)[0]
    fetch(`/api/v4/members/${urlName}/actions/block`, {
      method: 'POST',
      headers: new Headers({
        ...myLocalC.fetchHeaders,
        'x-xsrftoken': document.cookie.match(/(?<=_xsrf=)[\w-]+(?=;)/)[0],
