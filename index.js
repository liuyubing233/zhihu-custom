...
如果开启了「不再显示已屏蔽用户发布的内容」那么也不会看到对方发布的回答`;
  ...
  var range = (v, min, max, unit = "") => `<div class="ctz-flex-wrap ctz-range-${v}">${`<span class="ctz-commit" style="margin-right: 2px;">${min}${unit}</span><input class="ctz-i" type="range" min="${min}" max="${max}" name="${v}" style="width: 300px" /><span class="ctz-commit" style="margin: 0 8px 0 2px;">${max}${unit}</span><span>当前：<span id="${v}">0</span>${unit}</span>`}</div>`;
