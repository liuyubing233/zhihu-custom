...
如果开启了「不再显示已屏蔽用户发布的内容」那么也不会看到对方发布的回答`;
  ...
  var commonLabelCheckbox = (con) => con.map(
    ({ label, value, needFetch }) => `<label class="ctz-flex-wrap ${needFetch ? "ctz-fetch-intercept" : ""}">` + commonCheckbox(value) + commonText(label + (needFetch ? '<span class="ctz-need-fetch">（接口拦截已关闭，此功能无法使用）</span>' : "")) + `</label>`
  ).join("");
  var initHTML = () => {
