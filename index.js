...
屏蔽后，对方将不能关注你、向你发私信、评论你的实名回答、使用「@」提及你、邀请你回答问题，但仍然可以查看你的公开信息。`;
  ...
  var changeBlockedUsersBox = (isBlocked, showBlock, showBlockTag, showBlockTagType) => {
    if (isBlocked) {
      return fnReturnStr(`<span class="${CLASS_BLACK_TAG}">黑名单</span>`, showBlockTag) + fnReturnStr(`<button class="${CLASS_BTN_REMOVE_BLOCKED} ctz-button">解除屏蔽</button>`, showBlock);
    } else {
