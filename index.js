...
屏蔽后，对方将不能关注你、向你发私信、评论你的实名回答、使用「@」提及你、邀请你回答问题，但仍然可以查看你的公开信息。`;
  ...
  var blackItem = (info) => `<div class="ctz-black-item ctz-black-id-${info.id}" data-info='${JSON.stringify(info)}'>${blackItemContent(info)}</div>`;
  var blackItemContent = ({ id, name }) => `<a href="https://www.zhihu.com/people/${id}" target="_blank">${name}</a><i class="${CLASS_REMOVE_BLOCK}" style="margin-left:4px;cursor:pointer;font-style: normal;">✕</i>`;
  var initBlackList = async () => {
  ...
  function syncBlackList(offset = 0, l = []) {
    ...
    const buttonSync = domById(ID_BUTTON_SYNC_BLOCK);
