...
如果开启了「不再显示已屏蔽用户发布的内容」那么也不会看到对方发布的回答`;
  ...
  var initInputRange = () => {
    const createRangeInnerHTML = ({ label, value, min, max }) => `<div class="ctz-flex-wrap">${label ? `<div class="ctz-label">${label}</div>` : ""}<input class="ctz-i" type="range" min="${min}" max="${max}" name="${value}" style="width: 300px" /><span id="${value}" style="margin: 0 8px">0</span><span class="ctz-commit">滑动条范围: ${min} ~ ${max}</span></div>`;
    domById("CTZ_VERSION_RANGE_ZHIHU").innerHTML = VERSION_RANGE.map((item) => createRangeInnerHTML(item)).join("");
    domById("CTZ_IMAGE_SIZE_CUSTOM").innerHTML = createRangeInnerHTML({ label: "", value: "zoomImageSize", min: 0, max: 1e3 });
    domById("CTZ_LIST_VIDEO_SIZE_CUSTOM").innerHTML = createRangeInnerHTML({ label: "", value: "zoomListVideoSize", min: 0, max: 1e3 });
  };
  var initHTML = () => {
