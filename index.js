...
(function () {
  ...
  let pfConfig = {
    ...
    hiddenCollegeEntranceExamination: true, // 知乎高考（临时补充）
  ...
  function initHTML() {
    ...
    $($('.pf-pdf-box-content')[0].contentWindow.document.head).append(`<style type="text/css" id="pf-css-own">${INNER_CSS}</style>`)
  ...
  function collectionExport() {
    ...
    evenButton[0].onclick = function () {
      ...
      fetch(`/api/v4/collections/${id}/items?offset=${offset}&limit=20`, {
          ...
          switch (type) {
            case 'zvideo':
              return `<div class="pf-pdf-dialog-item">`
                + `<div class="pf-pdf-dialog-title">${title}</div>`
            ...
            default:
              return `<div class="pf-pdf-dialog-item">`
                + `<div class="pf-pdf-dialog-title">${title || question.title}</div>`
                + `<div>${content}</div>`
                + `</div>`
          }
        })
        ...
        this.innerText = '将当前页码详细内容生成为PDF'
        this.disabled = false
        iframe.contentWindow.print()
      })
    }
    $('.CollectionDetailPageHeader-title').append(evenButton)
  }
  ...
  function addQuestionCreatedAndModifiedTime() {
    ...
    if (pfConfig.questionCreatedAndModifiedTime) {
