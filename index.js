...
屏蔽后，对方将不能关注你、向你发私信、评论你的实名回答、使用「@」提及你、邀请你回答问题，但仍然可以查看你的公开信息。`;
  ...
  var initHTML = () => {
    ...
    dom("#CTZ_BASIC_SHOW_SELECT", nDomMain).innerHTML = SELECT_BASIS_SHOW.map(
      ({ label, value }) => createHTMLFormItem({ label, value: `<select class="ctz-select" name="${value}"></select>` })
    ).join("");
