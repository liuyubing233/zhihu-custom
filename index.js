...
(() => {
  ...
  var VERSION_RANGE_SHOW_VALUE = "当前: ";
  var VERSION_RANGE = [
    {
      label: "列表内容宽度",
      value: "versionHome",
...
如果开启了「不再显示已屏蔽用户发布的内容」那么也不会看到对方发布的回答`;
  ...
  var createHiddenItem = (arrHidden) => {
    ...
    return `<div class="ctz-checkbox-group">${arrHidden.map((item) => item.map((i) => `<label>${commonCheckbox(i.value)}<div>${i.label}</div></label>`).join("")).join("<br>")}</div>`;
  };
  var rangeHTML = (l, v, min, max, desc) => `<div class="ctz-form-item ctz-range-${v}"">${`<div>${l}</div><div>${range(v, min, max) + (desc ? `<div class="ctz-commit" style="margin-top: 8px;">${desc}</div>` : "")}</div>`}</div>`;
  var range = (v, min, max) => `<div class="ctz-flex-wrap">${`<span class="ctz-commit" style="margin-right: 2px;">${min}</span><input class="ctz-i" type="range" min="${min}" max="${max}" name="${v}" style="width: 300px" /><span class="ctz-commit" style="margin: 0 8px 0 2px;">${max}</span><span id="${v}">${VERSION_RANGE_SHOW_VALUE}0</span>`}</div>`;
  var commonLabelCheckbox = (con) => con.map(
    ({ label, value, needFetch }) => `<label class="ctz-flex-wrap ${needFetch ? "ctz-fetch-intercept" : ""}"><input class="ctz-i" name="${value}" type="checkbox" value="on" /><div>${label + (needFetch ? '<span class="ctz-need-fetch">（接口拦截已关闭，此功能无法使用）</span>' : "")}</div></label>`
  ).join("");
  var initHTML = () => {
    ...
    domById("CTZ_VERSION_RANGE_ZHIHU").innerHTML = VERSION_RANGE.map(
      (item) => `<div>${rangeHTML(item.label, item.value, item.min, item.max, item.desc) + rangeHTML(item.percentLabel, item.percentValue, item.percentMin, item.percentMax, item.desc) + `<div class="ctz-form-item">${`<div>${item.percentChooseLabel}</div><div><input class="ctz-i" name="${item.percentChooseValue}" type="checkbox" value="on" /></div>`}</div>`}</div>`
    ).join("");
