...
(function(){
  ...
  // src/commons/math-for-my-listens.ts
  ...
  var myBackground = {
    ...
    dark: (bg) => {
      ...
      const pageLearning = `.TopNavBar-fixMode-qXKMs,.index-tabWrap-4Smyx,.index-bannerItem-3o3D7,.LearningRouteCard-pathContent-j3jVv{background: ${bg}!important;}.LearningRouteCard-pathItem-xin1f .LearningRouteCard-content-kw2RW .LearningRouteCard-title-do7ND{color: ${t1}!important;}`;
    ...
    normalAppHeader: (bg) => {
      ...
      return `${headerBelongAd ? `.AppHeader:not(.${headerBelongAd})` : ".AppHeader"}{background-color:${BACKGROUND_CONFIG[bg].opacity}!important;background:${BACKGROUND_CONFIG[bg].opacity}!important;}`;
    }
  };
  var myCustomStyle = {
    init: function() {
  ...
  var myVersion = {
    ...
    vSusHeader: function() {
      const pfConfig = this.getConfig();
      return `.position-suspensionFind{${pfConfig.suspensionFindPo}}.position-suspensionUser{${pfConfig.suspensionUserPo}}.position-suspensionSearch{${pfConfig.suspensionSearchPo}}.position-suspensionFind .Tabs-link{border:1px solid #999999;color:#999999;background: ${pfConfig.colorBackground || "transparent"};}.position-suspensionFind .Tabs-link.is-active{color:#0066ff!important;border-color:#0066ff!important;}.position-suspensionUser .css-1m60na {display: none;}.position-suspensionUser .css-1n0eufo{margin-right: 0;}`;
    },
    /** 列表内容点击高亮边框 */
  ...
  var suspensionPackUp = (elements) => {
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      evenButton.style.cssText = needStyle ? `visibility:visible!important;position: fixed!important;bottom: 60px;right: ${(document.body.offsetWidth - even.offsetWidth) / 2 + RIGHT}px;box-shadow: 0 1px 3px rgb(18 18 18 / 10%);height: 40px!important;padding: 0 12px!important;background: ${myBackground.isUseDark() ? BACKGROUND_DARK_COLORS[colorBackground].b2 : BACKGROUND_CONFIG[colorBackground].opacity ? BACKGROUND_CONFIG[colorBackground].opacity : colorBackground}!important;` : "";
    }
  };
  ...
  // src/inner/init-history-view.ts
...
如果开启了「不再显示已屏蔽用户发布的内容」那么也不会看到对方发布的回答`;
  ...
  var initHTML = () => {
    ...
    nodeCTZBackground && (nodeCTZBackground.innerHTML = Object.keys(BACKGROUND_CONFIG).map((key) => {
      const { name, color } = BACKGROUND_CONFIG[key];
      return `<label class="ctz-bg-choose-label"><input class="${CLASS_INPUT_CLICK}" name="colorBackground" type="radio" value="${key}"/><div style="background: ${key};border: 2px solid ${key};color: ${color}">${name}</div></label>`;
    }).join(""));
  ...
  // src/methods/listen-list-item.ts
    ...
    init: async function() {
      ...
      for (let i = this.index, len = elements.length; i < len; i++) {
        ...
        if (highlightOriginal && dataZop && dataZop.authorName === userName && !message) {
          const highlight = `background: ${myBackground.isUseDark() ? `${BACKGROUND_DARK_COLORS[colorBackground].b2}!important;` : colorBackground === "#ffffff" ? "#fff3d4!important;" : `${colorBackground}!important;`}`;
  ...
  // src/inner/init-top-story-recommend.ts
  ...
  // src/methods/collection-export.ts
  ...
  // src/methods/listen-answer-item.ts
  ...
  // src/methods/listen-search-list-item.ts
  ...
  // src/inner/init-image-preview.ts
  ...
  // src/inner/init-link-changer.ts
  var initLinkChanger = () => {
  ...
  // src/inner/init-resize-observer.ts
  ...
  // src/methods/ctz-type-operate.ts
  ...
  // src/methods/page-filter-setting.ts
  ...
  (function() {
    ...
    window.addEventListener(
      ...
      async () => {
        ...
        fnLog(
          `加载完毕, 加载时长: ${(performance.now() - T0) / 1e3}s, 可使用 shift + . 或点击左侧眼睛按钮唤起修改器弹窗，如果快捷键不生效可以在控制台使用 window.openCtz() 唤起`
        );
      },
      false
    );
