...
(() => {
  ...
  var replaceBlockedCommentContent = (item, commentBoxClass, blockedUser, showBlockUserTagType) => {
    ...
    if (!commentContent || commentContent.classList.contains(CLASS_BLOCKED_CONTENT_REPLACEMENT2)) return;
    commentContent.innerHTML = BLOCKED_CONTENT_REPLACEMENT_TEXT2 + createBlockedUserTagHTML(showBlockUserTagType, blockedUser);
    commentContent.classList.add(CLASS_BLOCKED_CONTENT_REPLACEMENT2);
  ...
  var formatComments = async (nodeComments, commentBoxClass = ".css-jp43l4") => {
    ...
    const { removeBlockUserComment, replaceBlockUserContentWithStar, blockedUsers, showBlockUserComment, showBlockUserCommentTag, showBlockUserTagType } = await myStorage.getConfig();
