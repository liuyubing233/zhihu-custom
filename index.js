...
const ICO = {
  ...
  juejin: '<link data-n-head="ssr" rel="shortcut icon" id="pf-ico" href="https://lf3-cdn-tos.bytescm.com/obj/static/xitu_juejin_web//static/favicons/favicon-32x32.png">',
...
const BASIS_CHECKBOX_LIST = [
  { label: '问题显示创建和最后修改时间', value: 'questionCreatedAndModifiedTime' },
  { label: '首页列表显示创建与最后修改时间', value: 'listItemCreatedAndModifiedTime' },
  { label: '回答列表显示创建与最后修改时间（更改后请刷新页面）', value: 'answerItemCreatedAndModifiedTime' },
...
(function () {
  ...
  const versionCSS = {
    ...
    vHeartZhuanlan: function () {
      ...
      const heart = `.zhuanlan .Post-RichTextContainer,.zhuanlan .Post-NormalMain>div`
  ...
  const backgroundCSS = {
    ...
    chooseBG: function (bg) {
      return pfConfig.isUseThemeDark
        ? this.dark()
        : bg !== '#ffffff' ? this.normal(bg) : '.GlobalSideBar-navList{background: #fff}'
    },
    dark: () => {
      // 夜间模式
      ...
      const cF = `.pf-left li a,.css-1204lgo,.css-1ng3oge,.css-5abu0r,.css-p52k8h,.css-1dpmqsl,.css-1myqwel`
        ...
        + `,.css-kkim14,.css-1mx84bz,.RichContent-collapsedText,.ContentItem-arrowIcon`
        + `{color: #fff!important}`
    ...
    normal: (bg) => {
      ...
      const opacityBG = `.QuestionHeader,.Card,.HotItem,.Recommendations-Main,.GlobalSideBar-navList`
  ...
  const eventBlockHTML = (info) => {
    ...
    return `<div class="pf-block-item pf-block-id-${id}" data-info='${JSON.stringify(info)}'>`
      + `<img src="${avatar}" />`
  ...
  const eventCheckboxItemHTML = ({ value, label }) => {
    return `<div class="pf-checkbox-div" style="border-bottom: 1px solid #eee;padding: 4px 0;"><label>` +
