...
(() => {
  ...
  var createHTMLBackgroundSetting = () => {
  ...
  var createHTMLFormBoxSwitch = (con) => con.map(
    (item) => `<div class="ctz-form-box">${item.map(
      ({ label, value, needFetch, tooltip }) => `<div class="ctz-form-box-item ${needFetch ? "ctz-fetch-intercept" : ""}">${`<div>${label + (needFetch ? '<span class="ctz-need-fetch">（接口拦截已关闭，此功能无法使用）</span>' : "") + (tooltip ? createHTMLTooltip(tooltip) : "")}</div><div><input class="ctz-i ctz-switch" name="${value}" type="checkbox" value="on" /></div>`}</div>`
    ).join("")}</div>`
  ).join("");
  var createHTMLHiddenConfig = () => {
  ...
  var initHTMLBlockedUserTags = async () => {
    const config = await myStorage.getConfig();
  ...
  var initHTMLBlockedUsers = async () => {
    const config = await myStorage.getConfig();
...
屏蔽后，对方将不能关注你、向你发私信、评论你的实名回答、使用「@」提及你、邀请你回答问题，但仍然可以查看你的公开信息。`;
  ...
  var myVersion = {
    ...
    content: async function() {
      ...
      const xxxWidth = `.Topstory-mainColumn,.SearchMain{width: ${versionSizeHome}!important;}.Question-main,.QuestionHeader-footer-inner,.QuestionHeader .QuestionHeader-content{width: ${versionSizeAnswer}!important;}.Post-NormalMain .Post-Header,.Post-NormalMain>div,.Post-NormalSub>div,.zhuanlan .css-1xy3kyp,.zhuanlan .css-1voxft1,.zhuanlan .css-9w3zhd{width: ${versionSizeArticle}!important;}.zhuanlan .Post-SideActions{right: ${!versionArticleIsPercent ? `calc(50vw - ${+(versionArticle || "1000") / 2 + 150}px)` : `calc(50vw - ${+(versionArticlePercent || "70") / 2}vw + 150px)`}}.Topstory-mainColumn,.SearchMain,.Question-main,.QuestionHeader-footer-inner,.QuestionHeader .QuestionHeader-content,.Post-NormalMain .Post-Header,.Post-NormalMain>div,.Post-NormalSub>div,.${CLASS_ZHIHU_COMMENT_DIALOG},.Topstory-body .${CLASS_ZHIHU_COMMENT_DIALOG},.PostIndex-body .${CLASS_ZHIHU_COMMENT_DIALOG}{min-width: ${VERSION_MIN_WIDTH}px!important;}` + fnReturnStr(
        `.Topstory-body .${CLASS_ZHIHU_COMMENT_DIALOG}{width: ${versionSizeHome}!important;}.PostIndex-body .${CLASS_ZHIHU_COMMENT_DIALOG}{width: ${versionSizeArticle}!important;}` + fnReturnStr(`.${CLASS_ZHIHU_COMMENT_DIALOG}{width: ${versionSizeAnswer}!important;}`, location.pathname.includes("question")),
        commitModalSizeSameVersion
      );
  ...
  var initHTML = () => {
    ...
    domById("CTZ_VERSION_RANGE_ZHIHU").innerHTML = VERSION_RANGE.map(
      (item) => `<div class="ctz-form-box-item">${`<div>${item.label}${createHTMLTooltip(item.desc)}</div><div>${createHTMLRange(item.value, item.min, item.max) + createHTMLRange(item.percentValue, item.percentMin, item.percentMax, "%")}</div>`}</div><div class="ctz-form-box-item">${`<div>${item.percentChooseLabel}</div><div><input class="ctz-i ctz-switch" name="${item.percentChooseValue}" type="checkbox" value="on" /></div>`}</div>`
    ).join("");
    domById("CTZ_IMAGE_SIZE_CUSTOM").innerHTML = `<div>回答和文章图片宽度</div>` + createHTMLRange("zoomImageSize", 0, 1e3);
    domById("CTZ_IMAGE_HEIGHT_CUSTOM").innerHTML = `<div>图片最大高度</div>` + createHTMLRange("zoomImageHeightSize", 0, 1e3);
    domById("CTZ_LIST_VIDEO_SIZE_CUSTOM").innerHTML = `<div>列表视频回答宽度</div>` + createHTMLRange("zoomListVideoSize", 0, 1e3);
